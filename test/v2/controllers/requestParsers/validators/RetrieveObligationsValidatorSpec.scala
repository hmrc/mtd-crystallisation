...
class RetrieveObligationsValidatorSpec extends UnitSpec {
  val validator = new RetrieveObligationsValidator()
  ...
  "running a validator" should {
    "return FORMAT_NINO" when {
      "invalid nino" in {
        validator.validate(RetrieveObligationsRawData("A12344A", validFromDate, validToDate)) shouldBe
    ...
    "return FORMAT_FROM_DATE" when {
      "invalid from date invalid" in {
        validator.validate(RetrieveObligationsRawData(validNino, "xxxx", validToDate)) shouldBe
    ...
    "return FORMAT_TO_DATE" when {
      "invalid to date invalid" in {
        validator.validate(RetrieveObligationsRawData(validNino, validFromDate, "xxxx")) shouldBe
    ...
    "return MISSING_FROM_DATE" when {
      "from date missing" in {
        validator.validate(RetrieveObligationsRawData(validNino, "", "2019-01-01")) shouldBe
    ...
    "return RANGE_TO_DATE_BEFORE_FROM_DATE" when {
      "to is before from" in {
        validator.validate(RetrieveObligationsRawData(validNino, "2019-01-01", "2018-01-01")) shouldBe
    ...
    "return RANGE_DATE_TOO_LONG" when {
      "to date range is more than 366 days" in {
        validator.validate(RetrieveObligationsRawData(validNino, "2019-01-01", "2020-01-03")) shouldBe
    ...
    "return RULE_FROM_DATE_NOT_SUPPORTED" when {
      "from date is too early" in {
        validator.validate(RetrieveObligationsRawData(validNino, "2016-01-01", "2016-04-01")) shouldBe
    ...
    "return multiple errors" when {
      "multiple errors are detected" in {
        validator.validate(RetrieveObligationsRawData(validNino, "xxxx", "xxxx")) shouldBe
